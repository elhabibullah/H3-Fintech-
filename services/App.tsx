    ...
    return (
        <div className="h-[100dvh] bg-black flex flex-col relative overflow-hidden">
                 ...
                 <ChromeCard className="mb-8" rounded="rounded-[2rem]">
                     <div className="p-6 relative overflow-hidden group">
                         <div className="absolute top-0 right-0 p-4 opacity-50 group-hover:opacity-100 transition-opacity">
                             <div className="px-2 py-1 rounded border border-emerald-500/30 bg-emerald-900/10 text-[10px] text-emerald-400 uppercase tracking-wider">
                                 Active
                             </div>
                         </div>
                         ...
                         <div className="flex gap-3">
                             <Button onClick={() => navigate('/transfer')} className="flex-1 h-12 text-xs" variant="primary">
                                 <ArrowLeftRight size={16} /> {t.transfer}
                             </Button>
                             <Button onClick={() => navigate('/receive')} className="flex-1 h-12 text-xs" variant="secondary">
             ...
             <button 
                ...
                className="fixed bottom-24 right-6 w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-full shadow-[0_0_20px_rgba(16,185,129,0.4)] flex items-center justify-center text-white z-30 hover:scale-110 transition-transform active:scale-95"
             >
                     ...
                     <div className="w-full h-[85vh] sm:h-[600px] sm:max-w-md bg-zinc-950 border border-zinc-800 rounded-t-3xl sm:rounded-3xl flex flex-col shadow-2xl animate-in slide-in-from-bottom-20 duration-300" onClick={e => e.stopPropagation()}>
                         ...
                         <div className="p-4 border-b border-zinc-800 flex justify-between items-center bg-zinc-900/50 rounded-t-3xl">
                             <div className="flex items-center gap-3">
                                 ...
                                 <span className="font-bold text-slate-200 tracking-wide">{t.support}</span>
                             </div>
                         ...
                         <div className="flex-1 overflow-y-auto p-4 space-y-4">
                             {messages.length === 0 && (
                                 <div className="text-center py-12 text-slate-600">
                                     ...
                                     <p className="text-sm">How can Hura help you today?</p>
                                 </div>
                             )}
                             {messages.map(msg => (
                                 <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                                     <div className={`max-w-[80%] p-3 rounded-2xl text-sm leading-relaxed ${
                                         msg.role === 'user' 
                                         ? 'bg-emerald-900/30 text-emerald-100 rounded-tr-sm border border-emerald-500/20' 
                                         : 'bg-zinc-800 text-slate-200 rounded-tl-sm border border-zinc-700'
                                     }`}>
                                         {msg.text}
                                     </div>
                                 </div>
                         ...
                         <div className="p-4 border-t border-zinc-800 bg-zinc-900/50 rounded-b-3xl">
                             <form onSubmit={handleSendMessage} className="flex gap-2">
                                 <input 
                                    className="flex-1 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-emerald-500/50 transition-colors"
                                    placeholder={t.typeInquiry}
                                    value={chatInput}
                                    onChange={e => setChatInput(e.target.value)}
                                 />
                                 <button type="submit" disabled={!chatInput.trim() || isTyping} className="p-3 bg-emerald-600 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed hover:bg-emerald-500 transition-colors">
...
const LegalProfileScreen: React.FC = () => {
    const { user, logout, t, lang } = useAppContext();
    const navigate = useNavigate();

    return (
        <div className="h-[100dvh] bg-black flex flex-col relative">
            <div className="flex items-center justify-between mb-6 pt-8 px-6">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/')} className={`p-2 bg-zinc-900 rounded-full text-slate-400 hover:text-white transition-colors border border-slate-800`}>
                        <ArrowLeftRight size={20} className="rotate-180 rtl:rotate-0" /> 
                    </button>
                    <h1 className={`text-lg font-medium tracking-wide ${textMetal}`}>{t.legalPageTitle}</h1>
                </div>
                <button onClick={() => navigate('/edit-profile')} className="text-emerald-500 text-sm font-bold tracking-wider uppercase hover:text-emerald-400">{t.edit}</button>
            </div>

            <div className="flex-1 overflow-y-auto px-6 pb-32">
                 {/* Profile Card */}
                 <ChromeCard className="mb-6">
                     <div className="p-6">
                         <div className="flex items-center gap-4 mb-6">
                             <div className="w-16 h-16 bg-zinc-800 rounded-full flex items-center justify-center text-slate-400 border border-zinc-700">
                                 {user?.profileImage ? (
                         ...
                         <div className="grid grid-cols-1 gap-4 border-t border-zinc-800/50 pt-4">
                             <div>
                                 <p className="text-[10px] uppercase text-slate-500 tracking-widest mb-1">{t.legalName}</p>
                                 <p className="text-sm text-slate-300">{user?.name}</p>
                             </div>
                             <div>
                                 <p className="text-[10px] uppercase text-slate-500 tracking-widest mb-1">{t.legalAddress}</p>
                 ...
                 <div className="space-y-2">
                     <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 flex justify-between items-center">
                         <div className="flex items-center gap-3">
                             ...
                             <span className="text-sm font-medium text-slate-300">{t.kycVerified}</span>
                         </div>
                         <CheckCircle size={16} className="text-emerald-500" />
                     </div>
                     
                     <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 flex justify-between items-center opacity-50">
                         <div className="flex items-center gap-3">
                             <SmartphoneNfc size={20} className="text-slate-500" />
                             <span className="text-sm font-medium text-slate-300">{t.addToWallet}</span>
                         </div>
                         <ChevronRight size={16} className="text-slate-600" />
                     </div>

                     <div className="mt-8">
                         <Button variant="danger" onClick={logout} className="h-12 text-xs">
    ...
    return (
         <div className="h-[100dvh] bg-black flex flex-col relative">
            <div className="flex items-center gap-4 mb-6 pt-8 px-6">
                ...
                <h1 className={`text-lg font-medium tracking-wide ${textMetal}`}>{t.edit} {t.personalInfo}</h1>
            </div>

            <div className="flex-1 overflow-y-auto px-6 pb-24">
                <ChromeCard>
                    <div className="p-6 space-y-6">
                        <Input label={t.legalName} name="name" value={formData.name} onChange={handleChange} />
                        ...
                        <Input label={t.region} name="region" value={formData.region} onChange={handleChange} />
                        <Input label={t.country} name="country" value={formData.country} onChange={handleChange} />
                    </div>
                </ChromeCard>
                
                <div className="mt-6">
                    <Button onClick={handleSave} isLoading={loading} variant="mercury">
...
const CardsScreen: React.FC = () => {
    const { user, virtualCards, addVirtualCard, removeVirtualCard, toggleCardStatus, t, lang } = useAppContext();
    const navigate = useNavigate();
    const [isCreating, setIsCreating] = useState(false);
    
    // Creation State
    const [newNetwork, setNewNetwork] = useState<'VISA' | 'MASTERCARD'>('VISA');
    const [newDesign, setNewDesign] = useState<'SILVER' | 'BLACK' | 'GOLD'>('BLACK');
    const [loading, setLoading] = useState(false);
    ...
    const handleIssueCard = () => {
        setLoading(true);
        setTimeout(() => {
            addVirtualCard(newNetwork, newDesign);
            setLoading(false);
            setIsCreating(false);
        }, 1500);
    }
    ...
    const handleDeleteCard = () => {
        if (settingsCardId) {
            removeVirtualCard(settingsCardId);
            setSettingsCardId(null);
            setShowDeleteWarning(false);
        }
    }

    // Preview Card Object for creation mode
    const previewCard: VirtualCard = {
        id: 'preview',
        cardNumber: newNetwork === 'VISA' ? '4000123456789010' : '5000123456789010',
        expiry: '12/28',
        cvv: '123',
        holderName: user?.name.toUpperCase() || 'YOUR NAME',
        type: newNetwork,
        status: 'ACTIVE',
        color: newDesign
    };
        ...
        <div className="h-[100dvh] bg-black flex flex-col relative">
             <div className="flex items-center justify-between mb-6 pt-8 px-6">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className={`text-lg font-medium tracking-wide ${textMetal}`}>{isCreating ? t.issueCard : t.myCards}</h1>
                </div>
                {!isCreating && (
                    <button onClick={() => setIsCreating(true)} className="p-2 bg-emerald-900/30 text-emerald-500 rounded-full border border-emerald-500/30 hover:bg-emerald-900/50 transition-colors">
                        <Plus size={20} />
                    </button>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-32">
                {isCreating ? (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div>
                            <div className="mb-4 flex justify-center">
                                <div className="w-full max-w-[340px]">
                                    <CardVisual card={previewCard} showDetails={false} />
                                </div>
                            </div>
                            <p className="text-center text-xs text-slate-500 uppercase tracking-widest">Card Preview</p>
                        </div>

                        {/* Network Selection */}
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">{t.selectNetwork}</label>
                            <div className="grid grid-cols-2 gap-4">
                                <button 
                                    onClick={() => setNewNetwork('VISA')}
                                    className={`p-4 rounded-xl border flex items-center justify-center transition-all h-16 ${newNetwork === 'VISA' ? 'bg-zinc-800 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-zinc-900 border-zinc-800 opacity-50'}`}
                                >
                                    <VisaLogo className="h-6 text-white" />
                                </button>
                                <button 
                                    onClick={() => setNewNetwork('MASTERCARD')}
                                    className={`p-4 rounded-xl border flex items-center justify-center transition-all h-16 ${newNetwork === 'MASTERCARD' ? 'bg-zinc-800 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.15)]' : 'bg-zinc-900 border-zinc-800 opacity-50'}`}
                                >
                        ...
                        {/* Design Selection */}
                        <div className="space-y-3">
                            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1">{t.selectDesign}</label>
                            <div className="grid grid-cols-3 gap-3">
                                <button onClick={() => setNewDesign('SILVER')} className={`relative h-20 rounded-xl overflow-hidden border transition-all ${newDesign === 'SILVER' ? 'border-emerald-500 scale-105' : 'border-transparent opacity-60'}`}>
                                    <div className="absolute inset-0 bg-gradient-to-br from-slate-200 to-slate-400"></div>
                                    <span className="relative z-10 text-[10px] font-bold text-slate-800 p-2 block">Silver</span>
                                </button>
                                <button onClick={() => setNewDesign('BLACK')} className={`relative h-20 rounded-xl overflow-hidden border transition-all ${newDesign === 'BLACK' ? 'border-emerald-500 scale-105' : 'border-transparent opacity-60'}`}>
                                    <div className="absolute inset-0 bg-gradient-to-br from-zinc-800 to-black"></div>
                                    <span className="relative z-10 text-[10px] font-bold text-white p-2 block">Midnight</span>
                                </button>
                                <button onClick={() => setNewDesign('GOLD')} className={`relative h-20 rounded-xl overflow-hidden border transition-all ${newDesign === 'GOLD' ? 'border-emerald-500 scale-105' : 'border-transparent opacity-60'}`}>
                                    <div className="absolute inset-0 bg-gradient-to-br from-amber-200 via-amber-400 to-amber-600"></div>
                                    <span className="relative z-10 text-[10px] font-bold text-amber-900 p-2 block">Gold</span>
                                </button>
                            </div>
                        </div>

                        <div className="pt-4">
                            <Button onClick={handleIssueCard} isLoading={loading} variant="mercury">
                                {t.confirmIssuance}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        {virtualCards.length === 0 ? (
                            <div className="text-center py-20 opacity-50">
                                <CreditCard size={48} className="mx-auto mb-4 text-slate-600" strokeWidth={1} />
                                <p className="text-slate-400">No cards found.</p>
                                <Button className="mt-6 w-auto mx-auto" variant="ghost" onClick={() => setIsCreating(true)}>Create your first card</Button>
                            </div>
                        ) : (
                                ...
                                <div key={card.id} className="group relative">
                                    ...
                                    <div className="mt-4 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                                        <button 
                                            ...
                                            className={`flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium border transition-colors whitespace-nowrap ${card.status === 'FROZEN' ? 'bg-emerald-900/20 text-emerald-400 border-emerald-500/30' : 'bg-zinc-900 text-slate-300 border-zinc-700'}`}
                                        >
                                            {card.status === 'FROZEN' ? <Lock size={12} /> : <Lock size={12} />}
                                            {card.status === 'FROZEN' ? t.unfreezeCard : t.freezeCard}
                                        </button>
                                        <button 
                                            onClick={() => setSettingsCardId(card.id)}
                                            className="flex items-center gap-2 px-4 py-2 rounded-full text-xs font-medium bg-zinc-900 text-slate-300 border border-zinc-700 hover:text-white transition-colors whitespace-nowrap"
                                        >
            ...
            {/* Card Settings Modal */}
            {settingsCardId && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => { setSettingsCardId(null); setShowDeleteWarning(false); }}>
                    <div className="w-full max-w-sm bg-zinc-900 border border-zinc-800 rounded-t-3xl sm:rounded-3xl p-6 animate-in slide-in-from-bottom-10 duration-300" onClick={(e) => e.stopPropagation()}>
                        <div className="w-12 h-1 bg-zinc-700 rounded-full mx-auto mb-6"></div>
                        
                        {!showDeleteWarning ? (
                            <>
                                <h3 className={`text-lg font-medium mb-6 ${textMetal}`}>{t.settings}</h3>
                                <div className="space-y-2">
                                    <button 
                                        className="w-full flex items-center gap-4 p-4 rounded-xl bg-zinc-800/50 hover:bg-zinc-800 transition-colors text-left"
                                        onClick={() => {
                                            toggleCardStatus(settingsCardId);
                                            setSettingsCardId(null);
                                        }}
                                    >
                                        <div className="p-2 bg-emerald-900/20 text-emerald-400 rounded-lg"><Lock size={18}/></div>
                                        <span className="text-sm font-medium text-slate-200">
                                            {virtualCards.find(c => c.id === settingsCardId)?.status === 'FROZEN' ? t.unfreezeCard : t.freezeCard}
                                        </span>
                                    </button>
                                    
                                    <button 
                                        className="w-full flex items-center gap-4 p-4 rounded-xl bg-red-900/10 hover:bg-red-900/20 transition-colors text-left group"
                                        onClick={() => setShowDeleteWarning(true)}
                                    >
                                        <div className="p-2 bg-red-900/20 text-red-500 rounded-lg group-hover:bg-red-900/30"><Trash2 size={18}/></div>
                                        <span className="text-sm font-medium text-red-400 group-hover:text-red-300">{t.deleteCard}</span>
                                    </button>
                                </div>
                            </>
                        ) : (
                            <div className="text-center">
                                ...
                                <h3 className="text-xl font-bold text-red-500 mb-2">{t.warning}</h3>
                                <p className="text-slate-400 text-sm mb-8 px-4">{t.deleteConfirmation}</p>
                                
                                <div className="space-y-3">
                                    <Button variant="danger" onClick={handleDeleteCard}>
                                        {t.confirmDelete}
                                    </Button>
                                    <Button variant="ghost" onClick={() => setShowDeleteWarning(false)}>
                                        {t.cancel}
                                    </Button>
                                </div>
                            </div>
...
const TransferScreen: React.FC = () => {
    const { user, updateBalance, addTransaction, t, lang } = useAppContext();
    const navigate = useNavigate();
    ...
    const handleTransfer = (e: React.FormEvent) => {
        ...
        setTimeout(() => {
            ...
            addTransaction({
                ...
                amount: -val,
                date: new Date().toLocaleDateString(lang === 'ar' ? 'ar-SA' : 'en-US', { day: 'numeric', month: 'short' }),
                status: 'COMPLETED',
    ...
    return (
        <div className="h-[100dvh] bg-black flex flex-col relative">
             <div className="flex items-center gap-4 mb-8 pt-8 px-6">
                <button onClick={() => navigate(-1)} className={`p-2 bg-zinc-900 rounded-full text-slate-400 hover:text-white transition-colors border border-slate-800`}>
                    <ArrowLeftRight size={20} className="rotate-180 rtl:rotate-0" /> 
                </button>
                <h1 className={`text-lg font-medium tracking-wide ${textMetal}`}>{t.sendMoney}</h1>
            </div>
                    ...
                    <div className="p-6">
                        {success ? (
                            <div className="py-12 flex flex-col items-center animate-in zoom-in duration-300">
                                <div className="w-16 h-16 bg-emerald-900/20 rounded-full flex items-center justify-center border border-emerald-500/30 mb-4 text-emerald-500">
                                    <Check size={32} />
                                </div>
                                <h3 className="text-xl font-medium text-white mb-1">{t.transferSuccess}</h3>
                                <p className="text-slate-500 text-sm font-mono">{amount} {user?.currency}</p>
                            </div>
                        ) : (
                            <form onSubmit={handleTransfer} className="space-y-6">
                                <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 text-center mb-6">
                                    <p className="text-[10px] uppercase text-slate-500 tracking-widest mb-1">{t.availableBalance}</p>
                                    <p className="text-2xl font-light text-white font-mono">{user?.balance.toLocaleString()} {user?.currency}</p>
                                </div>
    ...
    return (
        <div className="h-[100dvh] bg-black flex flex-col relative">
             <div className="flex items-center gap-4 mb-8 pt-8 px-6">
                ...
                <h1 className={`text-lg font-medium tracking-wide ${textMetal}`}>{t.receiveMoney}</h1>
            </div>

            <div className="flex-1 overflow-y-auto px-6 flex flex-col items-center">
                <ChromeCard className="w-full max-w-sm mb-6">
                    <div className="p-8 flex flex-col items-center">
                        <div className="w-64 h-64 bg-white rounded-2xl p-4 mb-6 shadow-inner">
                            {/* Simulated QR Code */}
                            <div className="w-full h-full border-4 border-black border-dashed rounded-lg flex items-center justify-center relative overflow-hidden">
                        ...
                        <div className="text-center w-full space-y-4">
                            <div>
                                <p className="text-[10px] uppercase text-slate-500 tracking-widest mb-1">{t.recipient}</p>
                                <p className="text-lg font-medium text-white">{user?.name}</p>
                            </div>
                            <div className="bg-zinc-900 p-3 rounded-xl border border-zinc-800 flex items-center justify-between">
                                <div className="text-left">
                                    <p className="text-[9px] uppercase text-slate-500 tracking-widest mb-0.5">{t.iban}</p>
                                    <p className="text-xs font-mono text-slate-300">SA42 1000 0000 1234 5678 90</p>
                                </div>
                                <button className="p-2 text-slate-400 hover:text-white"><Copy size={16}/></button>
                            </div>
                        </div>
                    </div>
                </ChromeCard>
